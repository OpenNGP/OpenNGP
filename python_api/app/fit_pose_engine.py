...
def main(config_file):
    ...
    if exists(best_ckpt):
        ...
        for param in optim_params[0]['params']:
            param.requires_grad = False
        step = 1
        total_loss = 0
        best_ckpt = pjoin(config.exp_dir, 'ckpt_best_pose.pth.tar')
    else:
        total_loss = 0
    ...
    for epoch in range(config.max_epochs):
        ...
        for batch in dataset:
            ...
            if step % config.print_every == 0:
                ...
                if data_trans.pose_pertrub is not None:
                    trans_diff, ang_diff = data_trans.pose_refine.compare(data_trans.pose_pertrub)
                    writer.add_scalar("train/trans_diff", trans_diff, step)
                    writer.add_scalar("train/ang_diff", ang_diff, step)
